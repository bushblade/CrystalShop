...
describe('addMany', () => {
	it('appends a new item with the requested quantity', () => {
		const { store } = makeStore()
		store.getState().addMany(makeItem(), 3, 5)
		expect(store.getState().items).toEqual([{ ...makeItem(), quantity: 3 }])
	})

	it('appends a new item with quantity 1', () => {
		const { store } = makeStore()
		store.getState().addMany(makeItem(), 1, 3)
		expect(store.getState().items).toEqual([{ ...makeItem(), quantity: 1 }])
	})

	it('adds the requested quantity on top of an existing line', () => {
		const { store } = makeStore()
		store.getState().addMany(makeItem(), 2, 5)
		store.getState().addMany(makeItem(), 2, 5)
		expect(store.getState().items).toHaveLength(1)
		expect(store.getState().items[0].quantity).toBe(4)
	})

	it('clamps the combined quantity at maxQuantity', () => {
		const { store } = makeStore()
		store.getState().addMany(makeItem(), 4, 5)
		store.getState().addMany(makeItem(), 3, 5)
		expect(store.getState().items[0].quantity).toBe(5)
	})

	it('clamps a single requested quantity above the limit', () => {
		const { store } = makeStore()
		store.getState().addMany(makeItem(), 10, 3)
		expect(store.getState().items[0].quantity).toBe(3)
	})

	it('is a no-op when already at the cap', () => {
		const { store } = makeStore()
		store.getState().addMany(makeItem(), 1, 1)
		store.getState().addMany(makeItem(), 1, 1)
		expect(store.getState().items).toEqual([{ ...makeItem(), quantity: 1 }])
	})
	...
	it('records the per-item limit', () => {
		const { store } = makeStore()
		store.getState().addMany(makeItem(), 2, 2)
...
describe('setQuantity', () => {
	it('sets a quantity within the limit', () => {
		const { store } = makeStore()
		store.getState().addMany(makeItem(), 1, 5)
	...
	it('clamps a quantity above the limit', () => {
		const { store } = makeStore()
		store.getState().addMany(makeItem(), 1, 2)
...
describe('remove', () => {
	it('removes only the matching line and its limit', () => {
		const { store } = makeStore()
		store.getState().addMany(makeItem(), 1, 3)
		store.getState().addMany(makeItem({ id: 'product-2', name: 'Rose Quartz' }), 1, 1)
...
describe('persistence', () => {
	it('rehydrates items and limits on a new store over the same storage', () => {
		...
		first.getState().addMany(makeItem(), 1, 2)
		first.getState().addMany(makeItem({ id: 'product-2', name: 'Rose Quartz' }), 1, 1)
